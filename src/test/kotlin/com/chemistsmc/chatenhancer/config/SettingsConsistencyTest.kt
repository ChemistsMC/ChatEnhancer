...
import com.chemistsmc.chatenhancer.TestHelper.getFromJar
...
class SettingsConsistencyTest {
    ...
    fun shouldContainAllPropertiesWithSameDefaultValue() {
        ...
        configData.properties.forEach {
            assertThat("config.yml does not have property for $it", it.isPresent(yamlReader), equalTo(true))
            assertThat("config.yml does not have same default value for $it", it.determineValue(yamlReader), equalTo(it.defaultValue))
        }
    }
    ...
    fun shouldNotHaveUnknownProperties() {
        ...
        val keysInCode = configData.properties
            .map { it.path }
            .toSet()

        // when / then
        val difference = Sets.difference(keysInYaml, keysInCode)
        assertThat("config.yml has unknown properties", difference, isEmpty)
    }
}
